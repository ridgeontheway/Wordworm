...
class LocalDataManagementService {
    ...
    public async getRequestedWords(fileName: string, startWord: number, incrementValue: number) {
        return new Promise((resolve, reject) => {
            // Writing epub to .txt
            this.epubParserService.extractTo(this.LOCAL_STORAGE_PATH + fileName, this.LOCAL_PARSED_PATH, (err) => {

            })

            var bookLocations: Array<string> = this.getParsedFilesForBook(fileName)
            var wordsSelectedFromLocations = this.readFiles(bookLocations, startWord, incrementValue)
            resolve(wordsSelectedFromLocations)
        })
    }


    private async readFiles(bookLocations: Array<string>, startWord: number, incrementValue: number) {
            var wordsSelectedFromLocations: string = ""
            var wordsLeftToRetrieve = incrementValue

            for (let location of bookLocations) {
                var readingInfo: any = await this.getWordsFromParsedLocation(startWord, wordsLeftToRetrieve, this.LOCAL_PARSED_PATH + location)
        ...
        files.forEach(file => {
            ...
            strippedParsedFileName = strippedParsedFileName!.substring(strippedParsedFileName!.indexOf('-') + 1)
    ...
    private getWordsFromParsedLocation(startWord: number, wordsToRetrieve: number, fileLocation: string) {
        return new Promise((resolve, reject) => {
